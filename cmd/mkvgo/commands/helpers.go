...
var CmdUsage = map[string]string{
	...
	"cmaf-mpd":           "mkvgo cmaf-mpd -o <manifest.mpd> <rung-dir> [<rung-dir> ...] [--audio <dir>]... (DASH manifest over CMAF rungs an external encoder produced: each dir holds one rendition's init.mp4 + .m4s segments; rungs are checked segment-aligned first)",
	"cmaf-hls":           "mkvgo cmaf-hls -o <dir> <rung-dir> [<rung-dir> ...] [--audio <dir>]... (HLS master + one media playlist per rendition over CMAF rungs an external encoder produced; same rung directories as cmaf-mpd)",
