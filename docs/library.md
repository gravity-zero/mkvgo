...
**Metadata read options on the facade.** `OpenMeta`/`ReadMeta` take the same options as `reader`: `WithBitrate`, `WithTags`, `WithChapters`, `WithAttachments`, `WithoutAttachmentData` (payloads left on disk: `Data` nil, `DataPath`/`DataOffset`/`Size` set), `WithCues` (the cue points with their cluster positions; `CuePoint.TimeMs` is milliseconds - the reader converts the stored `CueTime`, which the file holds in `TimecodeScale` units, so it compares directly against every other millisecond the API returns. Before this was fixed the field held the raw units, which differ from milliseconds on any file whose timebase is not the 1 ms default: if your code compensated for that by scaling the value yourself, remove the compensation - the type never changed, so nothing will fail to compile and nothing will fail a test), `WithoutTailScan`, `WithInBandColourFallback`, `WithSampledKeyframes`, `WithKeyframeIndex` - so a caller never needs the `mkv/reader` package to read head-only what it wants.
...
| Sentinel | Refusal |
| --- | --- |
| `ErrShiftNotRepresentable` | The shift is smaller than one timecode unit at the file's TimecodeScale, so it rounds to no shift at all; the message names the smallest shift the file CAN express, which a caller cannot derive without its scale. A shift that is merely not an exact multiple of a unit is NOT refused - it is applied to the nearest one (within half a unit of the request), since no file can express finer than its own resolution. |
