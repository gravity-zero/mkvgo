...
| Sentinel | Refusal |
| --- | --- |
| `ErrShiftNotRepresentable` | The shift is not a whole number of timecode ticks at the file's TimecodeScale (or rounds to zero). The message names a shift that WOULD apply - the nearest representable one, or the smallest the file can express - because a caller cannot derive it without the file's scale, and on a fine timebase almost no round number of milliseconds is representable. |
