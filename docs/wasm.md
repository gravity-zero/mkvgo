...
| `cueHealth(input, opts?)` | `Uint8Array` **or `Blob`/`File`** (head-only, no cluster walk) | `CueHealthReport` - can the index seek video? Spots a seek-broken index (non-empty but with no video cue) and one too coarse to land near its target, in milliseconds, with the remedy. Cues on other tracks are counted, never held against a file: seeking never uses them |
