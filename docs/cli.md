...
**MP4/MOV sources route to the container's native mechanism**: the same flag and sign edit the track's presentation through its edit list (`edts`/`elst`) in the moov - no block is touched at all, so the repair is a few bytes whatever the file size. The write needs permission on the file alone: the rewritten moov is patched in place when its size does not change, rewritten at the tail when it sits there, and otherwise (faststart, moov at the head) the new moov is appended and the old one retired to a `free` box - 4 bytes flipped, crash-ordered (the new moov reaches disk before the flip, so an interrupted run keeps the original semantics). Track and movie durations follow the shift. The Matroska mode flags (`--in-place`/`--replace`/`--keep-backup`/`--deep-verify`/`--strict`/`--rollback-delta`) do not apply and are refused. Explicit refusals: presenting a track before the presentation start (MP4 cannot without trimming media), unknown tracks, fragmented MP4. Library form: `mp4.RetimeTracks` (same signature as `matroska.RetimeTracks`).
